 ...
 * `year`: Their current year of collegegit  (freshman, sophomore, junior, senior)
 *
 ...
 **/
function voter(number, name, address) {
	return {
		number: number,
		name: name,
		address: address
...
function copy(member) {
	var num = member.number;
	var nam = member.name;
	var add = member.address;
	return {
		number: num,
		name: nam,
		address: add
...
function find(user) {
	...
	team.forEach(memb=> {
		if (memb.number === number) mate = copy(memb);});
 ...
 */
function one(user) {
	if(typeof user !== 'string') { return result(false, 'parameter must be a string', []); }

	var teammate = find(user);
	if(teammate === null) {return result(false, 'team member not found', []); }
	return result(true, 'team member found', [teammate]);
}
